...
KNATIVE_HOST=$(vmtoolsd --cmd "info-get guestinfo.ovfEnv" | grep "guestinfo.knative_host" | awk -F 'oe:value="' '{print $2}' | awk -F '"' '{print $1}')
KNATIVE_SCHEME=$(vmtoolsd --cmd "info-get guestinfo.ovfEnv" | grep "guestinfo.knative_scheme" | awk -F 'oe:value="' '{print $2}' | awk -F '"' '{print $1}' | tr [:upper:] [:lower:])
KNATIVE_DISABLE_TLS=$(vmtoolsd --cmd "info-get guestinfo.ovfEnv" | grep "guestinfo.knative_disable_tls_verification" | awk -F 'oe:value="' '{print $2}' | awk -F '"' '{print $1}' | tr '[:upper:]' '[:lower:]')
KNATIVE_PATH=$(vmtoolsd --cmd "info-get guestinfo.ovfEnv" | grep "guestinfo.knative_path" | awk -F 'oe:value="' '{print $2}' | awk -F '"' '{print $1}')
