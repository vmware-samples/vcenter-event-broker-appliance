...
EVENT_ROUTER_K8S_TEMPLATE=/root/config/event-router/templates/event-router-k8s-template.yaml
EVENT_ROUTER_K8S_CONFIG=/root/config/event-router/event-router-k8s.yaml

# Apply YTT overlay
ytt --data-value-file bom=${VEBA_BOM_FILE} -f ${EVENT_ROUTER_K8S_TEMPLATE} > ${EVENT_ROUTER_K8S_CONFIG}

kubectl apply -f /root/config/event-router/event-router-clusterrole.yaml
