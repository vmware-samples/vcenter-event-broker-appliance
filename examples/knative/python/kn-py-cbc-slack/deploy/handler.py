...
def home():
 # Extract  VM Name From CloudEvent Data
    ...
    try:
        ...
        return "Could not for Webhook to Slack"
    app.logger.debug(f"Slack Webhook object is {slackWebhook}")
    ...
    # Get VM objectc from CBC instance, if not immediately available wait and retry
    tries = 5
    while tries >=0:
        try:
            ...
            for vm in cbcComputeResourceQuery:
                if vm.eligibility == 'ELIGIBLE':
                    ...
                    try:
                        ...
                        return "Request to install CB sensor failed", 400
