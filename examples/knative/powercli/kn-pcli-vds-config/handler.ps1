...
Function Process-Handler {
   ...
   if ( $vswitch.mtu -ne $enforcedMTU) {
      if(${env:FUNCTION_DEBUG} -eq "true") {
         Write-Host "MTU currently set to"$vswitch.mtu"- resetting to"$enforcedMTU
      }
      Write-Host "Resetting MTU to"$enforcedMTU
      Set-VDSwitch $vswitch -Mtu $enforcedMTU
   }
