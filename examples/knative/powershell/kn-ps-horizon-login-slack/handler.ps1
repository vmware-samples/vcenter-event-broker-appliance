...
Function Process-Handler {
   ...
   # Strip "https://" from source
   $horizonServer = ${cloudEvent}.source -replace "https://","" -replace "/",""

   # Construct Slack message object
   $payload = @{
      attachments = @(
         @{
            pretext = ":rotating_light: Failed login detected from Horizon Server: ${horizonServer} :rotating_light:";
            fields = @(
               @{
                     title = "Messsage";
