...
The `webhook` event provider listens for incoming CloudEvents (binary or
structured mode) on a configurable HTTP server in the VMware Event Router. The
HTTP method used to send the CloudEvent must be `POST`.
